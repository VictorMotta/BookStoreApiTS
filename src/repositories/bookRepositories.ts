...
async function findAll(): Promise<QueryResult<FindAllBooks>> {
...
async function findAllMyBooks(userId: number): Promise<QueryResult<findAllMyBooks>> {
  return await connectionDb.query(
    `
    SELECT 
      u.name as "userName",
...
async function create(name: string, author: string, userId: number): Promise<QueryResult> {
  return await connectionDb.query(
    `
